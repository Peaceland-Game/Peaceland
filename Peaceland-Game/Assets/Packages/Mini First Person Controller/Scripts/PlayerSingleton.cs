...
public class PlayerSingleton : MonoBehaviour
{
    ...
    public void StartLockPicking(Door door)
    {
       ...
       // transform.localRotation = Quaternion.LookRotation(lookDir, Vector3.up);
        ...
        var lockPos = new Vector3(activeLock.transform.position.x, 0, activeLock.transform.position.z);
        var plyPos = new Vector3(transform.position.x, 0, transform.position.z);

        activeLock.transform.localRotation = Quaternion.LookRotation(lockPos - plyPos, transform.up);

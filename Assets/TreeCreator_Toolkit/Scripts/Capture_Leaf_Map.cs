...
public class Capture_Leaf_Map : EditorWindow
{
    public LM_Render_Mode renderMode;
    ...
    [MenuItem("Window/Capture Leaf Map")]
    static void Init()
    {
        // Display window
        Capture_Leaf_Map window = (Capture_Leaf_Map)EditorWindow.GetWindow(typeof(Capture_Leaf_Map));
        window.Show();
    }
    public void Awake()
    {
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label("Capture Leaf Map - Update 1.0 March 2024", EditorStyles.helpBox);

        EditorGUILayout.Space();


        renderMode = (LM_Render_Mode)EditorGUILayout.EnumPopup("Render Mode", renderMode, GUILayout.Width(343));
        ...
        */
        ...
        if (GUILayout.Button("Render"))
        {
            ...
            string path = EditorUtility.SaveFilePanelInProject("Save As ...", "LeafMap_"
                + (UnityEditor.EditorPrefs.GetInt("LM_Number").ToString())
                , "png", "");

            if (renderMode == LM_Render_Mode.Reliable)
            {
            ...
            if (renderMode == LM_Render_Mode.Simple)
            {
                if (lmResolution == LM_Resolution._256)
                    Simple_Render_Billboard(targetCamera, 256, 256, path);
                if (lmResolution == LM_Resolution._512)
                    Simple_Render_Billboard(targetCamera, 512, 512, path);
                if (lmResolution == LM_Resolution._1024)
                    Simple_Render_Billboard(targetCamera, 1024, 1024, path);
                if (lmResolution == LM_Resolution._2048)
                    Simple_Render_Billboard(targetCamera, 2048, 2048, path);
                if (lmResolution == LM_Resolution._4096)
                    Simple_Render_Billboard(targetCamera, 4096, 4096, path);
            }
            UnityEditor.EditorPrefs.SetInt("LM_Number", UnityEditor.EditorPrefs.GetInt("LM_Number") + 1);
